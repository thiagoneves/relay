...
test run dumps 600 lines into the context window and the part that mattered
scrolls past.

relay sits under the agent, in the hooks your harness already calls.
...
**Long output, short context.** A 400-line report reached the model as 918
tokens instead of 3,248 in our own test, with errors, failing tests and
`file:line` references intact. The original is one `relay get <id>` away.
Replayed over 21,000 real shell calls, relay kept 100% of the signal lines.
...
**Nothing it takes from you.** No model calls in the default path, so the layer
costs zero tokens of your quota. No daemon, no network. Hooks run in about
...
```
[relay 8.2k→1.1k tokens · original: relay get o_1a0c9bd1f7d]
```
...
masked before relay stores them.
...
- **Claude Code** runs every command as the agent wrote it, under your own
  permission rules and mode, and relay swaps the output for its compressed view
  afterwards. Tests, builds, linters and reads also go through `relay x`, which
  relay approves, so their output is compressed even when they fail, and stops
  just before the harness's timeout so what was printed still reaches you.
- **Codex, Gemini CLI and Cursor** accept a rewritten command but cannot have
  its output replaced, so relay compresses the same reads and routine
