...
//!
//! Two sources: the local output store (this worktree's real history)
//! and a corpus directory of `<name>.cmd` + `<name>.out` fixtures, with
//! an optional `<name>.keep` listing lines that must survive verbatim.
...
fn sample(name: String, cmd: &str, raw: &str, expect: &[String], refetched: bool) -> Sample {
...
pub fn from_store(paths: &Paths) -> Vec<Sample> {
    ...
    outputs::list(paths)
        ...
        .filter_map(|m| {
            ...
            Some(sample(m.id.clone(), &m.cmd, &raw, &[], fetched.contains(&m.id)))
        })
        .collect()
}
    ...
    names
        ...
        .map(|name| {
            ...
            let expect: Vec<String> = read("keep")
                ...
                .map(str::to_string)
                .collect();
            Ok(sample(name, &cmd, &raw, &expect, false))
        })
        .collect()
}

/// Per-filter totals, sorted by filter name, plus the overall total.
pub fn totals(samples: &[Sample]) -> (Vec<Totals>, Totals) {
    let mut by: BTreeMap<String, Totals> = BTreeMap::new();
    let mut all = Totals { filter: "total".into(), ..Totals::default() };
    for s in samples {
        by.entry(s.filter.clone()).or_insert_with(|| Totals { filter: s.filter.clone(), ..Totals::default() }).add(s);
        all.add(s);
    }
    (by.into_values().collect(), all)
}
